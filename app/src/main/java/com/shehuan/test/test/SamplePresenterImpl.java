...
import com.shehuan.test.easymvp.net.CommonObserver;
...
public class SamplePresenterImpl extends BasePresenter<SampleContract.View> implements SampleContract.Presenter {
    ...
    public void getBannerData() {
        Disposable disposable = RequestManager.getInstance()
                .execute(RetrofitManager.getInstance().create(CommonApis.class).banner(), new CommonObserver<List<BannerBean>>(mContext, true) {
                    @Override
    ...
    public void getFriendData() {
        Disposable disposable = RequestManager.getInstance()
                .execute(RetrofitManager.getInstance().create(CommonApis.class).friend(), new CommonObserver<List<FriendBean>>(mContext, true) {
                    @Override
