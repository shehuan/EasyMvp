package com.shehuan.mvp.easymvp.base.fragment;
...
public abstract class BaseMvpFragment<P extends BasePresenter> extends BaseFragment {
    ...
    private boolean isParentVisible() {
        ...
        return fragment == null || (fragment instanceof BaseMvpFragment && ((BaseMvpFragment) fragment).isVisibleToUser);
    }
    ...
    private void dispatchParentVisibleState() {
        ...
        for (Fragment child : fragments) {
            if (child instanceof BaseMvpFragment && ((BaseMvpFragment) child).isVisibleToUser) {
                ((BaseMvpFragment) child).tryLoadData();
            }
        }
    }
    ...
    private void dispatchParentHiddenState() {
        ...
        for (Fragment child : fragments) {
            if (child instanceof BaseMvpFragment && !((BaseMvpFragment) child).isHidden) {
                ((BaseMvpFragment) child).tryLoadData1();
            }
        }
    }
    ...
    private boolean isParentHidden() {
        ...
        if (fragment == null) {
            return false;
        } else if (fragment instanceof BaseMvpFragment && !((BaseMvpFragment) fragment).isHidden) {
            return false;
        }
        return true;
    }
